; ----------------------------------------------------------------------------
; crime_of_passion.mc - instrumental homicide genesis: obsession (one-sided crave).
;
; Sibling of covet_inheritance.mc / ambition.mc / predation.mc. An actor consumed
...
; clear-marriage / rid-of-spouse branches are betrayal_kill.mc / clear_marriage.mc
; / rid_of_spouse.mc). The selection that run_generative_obsession dispatched is
; expressed here:
...
;     - crave is the fallback, so a known betrayal routes to betrayal_kill.mc instead
...
(npc-think crime_of_passion
  ...
  (role @self 
    ...
    (role ?beloved {?beloved isa [k human], condition [k alive]}
      ...
      ; FALLBACK - a discovered betrayal routes to betrayal_kill.mc.
