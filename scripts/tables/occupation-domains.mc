; ----------------------------------------------------------------------------
; occupation_domains.mc - the `domain` a job CONFERS competence in, as authored
; config (a (define-table ...), like businesses / cornerstone_businesses). Loaded
; from tables/ into the .mc catalog; read by derive_skills (C++) via
...
;             object kind). Under the org-pos > job tree.
;   domain  - a `domain` leaf (a bare atom, resolved to the domain kind by the
;             reader). S4 derive_skills grows {@self skilled-in <domain>} from
;             tenure in this job.
;
