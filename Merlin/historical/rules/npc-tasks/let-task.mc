...
;   CREATE_ENTITY [k for-lease-listing] : pen the listing;
...
(npc-task {@self LET ?prop}:?let-rel
  ...
  (and
    (try
      (when -{@self CREATE_ENTITY [k for-lease-listing] /succ /caused_by ?let-rel})
      (utility fallback)
      (effects (debug-print "LET_PEN")
               (maintain-proposal {@self CREATE_ENTITY [k for-lease-listing]})))
    (try
