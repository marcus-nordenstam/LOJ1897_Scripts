...
PROBE_RESIDUAL, PROBE_JOIN each fire once per NPC (49 at 50-pop), zero ERROR lines.

parse_errors/ holds one-event dirs that must each ABORT the load with a named
message (run each file alone as its own --events dir): inner /their-mind, inner
(fan-out ?x), clause nesting deeper than k_max_clause_depth. bad_inner_expr.hs is
the exception: it must LOAD (an inner op-expr in an enumerated role routes to the
