...
;   invest_action : the promoted 45-min proposal - records {@self backed_by <org>}
;                   (the org bound off the act pattern) + ends the act + the aim.
; ----------------------------------------------------------------------------

; The 45-min proposal, promoted from the back aim at the firm; matched by its (when) on
; the promoted {@self back} belief.
(npc-action {@self back ?org}
  (duration 45)
  (effects
    (begin-belief {@self backed_by ?org})
    (set-outcome {@self back} succ)))
