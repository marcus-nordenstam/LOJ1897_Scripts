...
; stepping rung. Each leg /causes the enter task, inheriting its drive - which inherits the desire
...
; The spatial predicates (in-building / at-threshold) stay in (when); the
; mutually-exclusive OUTSIDE vs AT-THRESHOLD gates make the threshold->interior handoff
