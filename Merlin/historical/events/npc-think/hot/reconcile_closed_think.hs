...
;
; This rule fires the cycle he is at the shuttered premises. Ending the employer belief makes
; the (role ?org (believes {@self employer ?org})) gate fail next cycle, so the rule
