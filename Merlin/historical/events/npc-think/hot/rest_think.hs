...
(npc-think seek_rest
  ...
  (effects       (target {@self home ?}): ?go_dest (debug-print "TRACE-SEEKREST home=?go_dest") (maintain-proposal {@self enter ?go_dest})))
...
(npc-think idle_go_home
  (when (not (at-home)))
  (utility 1)
  (effects       (target {@self home ?}): ?go_dest (debug-print "TRACE-IDLEHOME home=?go_dest") (maintain-proposal {@self enter ?go_dest})))
