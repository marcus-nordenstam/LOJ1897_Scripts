...
(define-macro conduct-scalar (?who ?dim)
  (if (any {?who ?dim [k conduct_level good]} (out int)) (then 0.85)
  (else (if (any {?who ?dim [k conduct_level lax]} (out int))  (then 0.25)
      (else 0.65)))))

; devoutness-scalar - the piety_band -> 0..1 (devout 0.85, secular 0.25, else 0.65).
(define-macro devoutness-scalar (?who)
  (if (any {?who devoutness [k piety_band devout]} (out int))  (then 0.85)
  (else (if (any {?who devoutness [k piety_band secular]} (out int)) (then 0.25)
      (else 0.65)))))
...
(define-macro decorum-scalar (?who)
  (if (any {?who decorum ?} (out int)) (then (any {?who decorum}).target) (else 0.65)))
...
(define-macro rootedness ()
  (clamp (+ (* 0.15 (any {@self mother ?} (out int)))
            (* 0.15 (any {@self father ?} (out int)))
...
(define-macro sobriety ()
  (clamp (+ (* (- 1 (any {@self craving ?} (out int))) (- 1 (attr @self intoxication)))
            (* (any {@self craving ?} (out int))       (min (- 1 (attr @self intoxication)) 0.15))
...
(define-macro inhibition ()
  (clamp (+ (+ (* (attr @self politeness)      0.30)
               ...
               (* (piety)                       0.20)
               (* (if (any {@self decorum ?} (out int)) (then (any {@self decorum}).target) (else 0)) 0.10)
               (* (/ (+ (- 1 (attr @self industriousness)) (- 1 (attr @self politeness))
                        (attr @self volatility)) 3) -0.20)
               (* (if (any {@self stress ?} (out int))  (then (any {@self stress}).target)  (else 0)) -0.30))
