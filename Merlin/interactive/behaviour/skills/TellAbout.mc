...
#
# The directive belief {?asker goal {@self tell_about ?subject ?asker}} is
...
{!@self:?asker goal {@self tell_about ?subject ?asker}}: ?directive
(bb_private_none ?subject tell_payload)
(none {@self /succ TELL ? ?asker /causes ~?directive})
...
# ?directive isn't a self-act, so its causal link must be added explicitly.
(add_causes ?response ?directive)
